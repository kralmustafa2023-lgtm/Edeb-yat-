  ...
  return (
    <div className="bg-surface-container-lowest min-h-screen relative flex items-center justify-center overflow-hidden font-['Inter']">
      ...
      <main className="relative z-10 w-full max-w-lg px-6">
        <div className="glass-panel rounded-[2rem] p-8 md:p-10 flex flex-col items-center">
          <header className="text-center mb-8">
            <h1 className="text-2xl md:text-3xl font-black text-amber-950 leading-tight mb-3 tracking-tighter">
          ...
          <div className="flex bg-amber-950/5 p-1.5 rounded-full w-full max-w-sm mb-8">
            <button 
              type="button"
              onClick={() => setRole('ogrenci')}
              className={`flex-1 py-2.5 px-6 rounded-full font-bold text-sm shadow-sm transition-all duration-300 ${role === 'ogrenci' ? 'bg-white text-amber-950' : 'text-amber-900/40 hover:text-amber-900/60'}`}
            >
              Öğrenci
            </button>
            <button 
              ...
              className={`flex-1 py-2.5 px-6 rounded-full font-bold text-sm transition-all duration-300 ${role === 'ogretmen' ? 'bg-white text-amber-950 shadow-sm' : 'text-amber-900/40 hover:text-amber-900/60'}`}
            >
              Öğretmen
            </button>
          </div>
          ...
          <form className="w-full max-w-sm space-y-5" onSubmit={handleLogin}>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  className="w-full pl-11 pr-5 py-3.5 rounded-xl input-ivory text-amber-950 placeholder-amber-900/30 focus:ring-2 focus:ring-amber-900/20 focus:border-transparent outline-none transition-all text-sm" 
            ...
            <div className="pt-3">
              <button 
                className="w-full bg-amber-900 hover:bg-amber-950 text-amber-50 font-black tracking-[0.15em] py-4 rounded-xl transition-all duration-300 flex items-center justify-center gap-3 group shadow-xl shadow-amber-950/20 text-sm" 
                type="submit"
              >
                SİSTEME GİRİŞ YAP
